...
-- ============================================
-- Fullscreen persistence across focus changes
-- ============================================
-- Keep window-fullscreen (maximized, mode 1) across focus switches, let
-- real fullscreen (mode 2) drop to tiling as before (old Hyprland default).
-- Hyprland drops ANY fullscreen flag the moment focus leaves a window, so
-- maximized windows need a re-apply on refocus. The fullscreen-cleared
-- event is ambiguous — it fires both on user dismiss and on focus move,
-- before the focus-change active event (0.56 experiment) — hence the
-- deferred decision: a cleared maximized window goes pending; shortly
-- after, one that lost focus was cleared by the switch (remember), a
-- still-focused one was user-dismissed (forget).

local fs_mode      = {}  -- address -> maximized mode seen at set time (always 1)
local fs_remember  = {}  -- address -> mode to restore
local fs_pending   = {}  -- address -> true (cleared, undecided)

local function fs_forget(addr)
    fs_mode[addr] = nil
    fs_remember[addr] = nil
    fs_pending[addr] = nil
end
...
hl.on("window.fullscreen", function(w)
    ...
    local was_maximized = fs_mode[addr] == 1
    fs_forget(addr)
    if w.fullscreen == 0 and was_maximized then
        -- cleared maximized window: focus-move or user dismiss? decide in a tick
        fs_pending[addr] = true
        hl.timer(function()
            if not fs_pending[addr] then return end
            fs_pending[addr] = nil
            local active = hl.get_active_window()
            if active == nil or active.address ~= addr then
                fs_remember[addr] = 1   -- focus moved away: keep maximized
            end
            -- still focused: user dismissed it, stay forgotten
        end, { timeout = 120, type = "oneshot" })
    end
    -- real fullscreen (2): untracked, focus move drops it to tiling (old default)
end)

hl.on("window.active", function(w)
    if w == nil then return end
    local mode = fs_remember[w.address]
    if mode ~= nil and w.fullscreen == 0 then
